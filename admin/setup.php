...
/**
 * \file    dashcycles/admin/setup.php
 * \ingroup dashcycles
 * \brief   DashCycles setup page.
 */
...
require_once '../lib/dashcycles.lib.php';
//require_once "../class/myclass.class.php";

// Translations
$langs->loadLangs(array("admin", "dashcycles@dashcycles"));

// Initialize technical object to manage hooks of page. Note that conf->hooks_modules contains array of hook context
$hookmanager->initHooks(array('dashcyclessetup', 'globalsetup'));
...
// $item = $formSetup->newItem('DASHCYCLES_MYPARAM1');
...
//$formSetup->newItem('DASHCYCLES_MYPARAM7')->setAsSecureKey();

// Setup conf for a selection of a product
// $formSetup->newItem('DASHCYCLES_MYPARAM8')->setAsProduct();
...
// $item->helpText = $langs->transnoentities('DASHCYCLES_MYPARAM10');

// Setup conf for a category selection
// $formSetup->newItem('DASHCYCLES_CATEGORY_ID_XXX')->setAsCategory('product');

// Setup conf DASHCYCLES_MYPARAM10
// $item = $formSetup->newItem('DASHCYCLES_MYPARAM10');
// $item->setAsColor();
...
if ($action == 'updateMask') {
	...
	foreach ($dirmodels as $reldir) {
		$file = dol_buildpath($reldir."core/modules/dashcycles/doc/pdf_".$modele."_".strtolower($tmpobjectkey).".modules.php", 0);
		if (file_exists($file)) {
			$filefound = 1;
	...
	if ($filefound) {
		...
		if ($module->write_file($tmpobject, $langs) > 0) {
			header("Location: ".DOL_URL_ROOT."/document.php?modulepart=dashcycles-".strtolower($tmpobjectkey)."&file=SPECIMEN.pdf");
			return;
		} else {
	...
	if (!empty($tmpobjectkey)) {
		$constforval = 'DASHCYCLES_'.strtoupper($tmpobjectkey)."_ADDON";
	...
	if ($ret > 0) {
		if (!empty($tmpobjectkey)) {
			$constforval = 'DASHCYCLES_'.strtoupper($tmpobjectkey).'_ADDON_PDF';
...
$help_url = '';
$title = "DashCyclesSetup";

llxHeader('', $langs->trans($title), $help_url, '', 0, 0, '', '', '', 'mod-dashcycles page-admin');
...
$head = dashcyclesAdminPrepareHead();
print dol_get_fiche_head($head, 'settings', $langs->trans($title), -1, "dashcycles@dashcycles");

// Setup page goes here
echo '<span class="opacitymedium">'.$langs->trans("DashCyclesSetupPage").'</span><br><br>';
					...
					while (($file = readdir($handle)) !== false) {
						if (strpos($file, 'mod_'.strtolower($myTmpObjectKey).'_') === 0 && substr($file, dol_strlen($file) - 3, 3) == 'php') {
							...
							if ($module->isEnabled()) {
								...
								$constforvar = 'DASHCYCLES_'.strtoupper($myTmpObjectKey).'_ADDON';
							...
							if (preg_match('/\.modules\.php$/i', $file) && preg_match('/^(pdf_|doc_)/', $file)) {
								if (file_exists($dir.'/'.$file)) {
									...
									if ($modulequalified) {
										...
										// Default
										print '<td class="center">';
										$constforvar = 'DASHCYCLES_'.strtoupper($myTmpObjectKey).'_ADDON_PDF';
