<?php
/* Copyright (C) 2025 LENOBLE Arthur <arthurl52100@gmail.com>
 *
...
/**
 * \file    custom/dashcycles/interface_99_modDashCyles_DashCyclesTriggers.class.php
 * \ingroup dashcycles
 * \brief   
 */
 ...
 */
class InterfaceDashCyclesTriggers extends DolibarrTriggers
{
	/**
	 * Constructor
	 *
	...
	public function __construct($db)
	{
		...
		$this->family = "dashcycles";
		$this->description = "Dashcycles triggers for suppliers orders status.";
		$this->version = 1.0;
		$this->picto = 'dashcycles@dashcycles';
	}
	...
	public function runTrigger($action, $object, User $user, Translate $langs, Conf $conf)
	{
		...
		switch ($action) {
			...
			case 'ORDER_SUPPLIER_CREATE':
				// Add a new line in dash SQL Table
				$sql = "INSERT INTO ".MAIN_DB_PREFIX."dashcycles (fk_commande, statut) VALUES (".$object->id.", 0)";
				$resql = $this->db->query($sql);
				if (!$resql){
					dol_syslog("ERROR in DashCyclesTrigger while creating a new order supplier");
				}
				break;
			...
			case 'ORDER_SUPPLIER_DELETE':
				// Cases where the line needs to be deleted from the dash SQL Table
				$sql = "DELETE FROM ".MAIN_DB_PREFIX."dashcycles WHERE fk_commande = ".$object->id;
				$resql = $this->db->query($sql);if (!$resql){
					dol_syslog("ERROR in DashCyclesTrigger while deleting a line in dashcycles table");
				}
				break;
